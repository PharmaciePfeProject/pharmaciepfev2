  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) setError(err?.response?.data?.message || "Failed to load inventories.");
      } finally {
    ...
    <div className="space-y-6">
      <div className="rounded-xl border bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex justify-end gap-3">
          <Button variant="outline" className="rounded-xl" onClick={() => { setProductId(""); setLocationId(""); setStateId(""); setUserId(""); setDateFrom(""); setDateTo(""); setSearchParams(new URLSearchParams({ page: "1", pageSize: String(currentPageSize) })); }}>Reset</Button>
          <Button className="rounded-xl" onClick={applyFilters}>Apply</Button>
        </div>
      </div>

      {loading && <div className="rounded-xl border bg-white p-8 text-center text-muted-foreground shadow-sm">Loading inventories...</div>}
            ...
            <table className="table-auto w-full border-collapse text-sm">
              <thead className="bg-muted/50">
                <tr className="text-left">
                  <th className="px-4 py-3 text-sm font-semibold">Inventory ID</th>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.inventory_id} className="border-t hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium">{item.inventory_id}</td>
                    <td className="px-4 py-3">{item.date_inv ? new Date(item.date_inv).toLocaleString() : "Not available"}</td>
                    ...
                    <td className="px-4 py-3"><Link to={`/app/inventories/${item.inventory_id}`}><Button size="sm" variant="outline" className="rounded-xl">View</Button></Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
