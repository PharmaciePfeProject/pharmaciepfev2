...
import { fetchInternalOrders } from "@/api/supply-flow";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) setError(err?.response?.data?.message || "Failed to load internal orders.");
      } finally {
    ...
    <div className="space-y-6">
      <div className="rounded-xl border bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex justify-end gap-3">
          <Button variant="outline" className="rounded-xl" onClick={() => { setProductId(""); setEmplacementId(""); setStateId(""); setTypeId(""); setDateFrom(""); setDateTo(""); setSearchParams(new URLSearchParams({ page: "1", pageSize: String(currentPageSize) })); }}>Reset</Button>
          <Button className="rounded-xl" onClick={applyFilters}>Apply</Button>
        </div>
      </div>
      {loading && <div className="rounded-xl border bg-white p-8 text-center text-muted-foreground shadow-sm">Loading internal orders...</div>}
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.internal_order_id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">{item.order_number || "Not available"}</td>
                    <td className="px-4 py-3">{item.order_date ? new Date(item.order_date).toLocaleString() : "Not available"}</td>
                    ...
                    <td className="px-4 py-3">{item.type_id ?? "Not available"} / {item.type_label || "Not available"}</td>
                    <td className="px-4 py-3">{item.lines.length}</td>
                    <td className="px-4 py-3"><Link to={`/app/internal-orders/${item.internal_order_id}`}><Button size="sm" variant="outline" className="rounded-xl">View</Button></Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
