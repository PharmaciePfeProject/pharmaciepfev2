...
import { fetchExternalOrders } from "@/api/supply-flow";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active) setError(err?.response?.data?.message || "Failed to load external orders.");
      } finally {
    ...
    <div className="space-y-6">
      <div className="rounded-xl border bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex justify-end gap-3">
          <Button variant="outline" className="rounded-xl" onClick={() => { setProductId(""); setEmplacementId(""); setStateId(""); setDateFrom(""); setDateTo(""); setSearchParams(new URLSearchParams({ page: "1", pageSize: String(currentPageSize) })); }}>Reset</Button>
          <Button className="rounded-xl" onClick={applyFilters}>Apply</Button>
        </div>
      </div>

      {loading && <div className="rounded-xl border bg-white p-8 text-center text-muted-foreground shadow-sm">Loading external orders...</div>}
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.external_order_id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3"><Link to={`/app/external-orders/${item.external_order_id}`}><Button size="sm" variant="outline" className="rounded-xl">View</Button></Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
